...
(defvar prove-mode-hook nil
  "Hook run when prove mode starts")
...
(defvar prove-mode-font-lock-keywords
  '(("^\\(not ok\\) \\([[:digit:]]+\\)" . 'prove-mode-not-ok-face)
    ...
    ;; this is for the running of prove without -v
    ("\\.\\{2,\\} \\(ok\\)$" 1 'prove-mode-ok-face))
...
(define-compilation-mode prove-mode "Prove"
  ...
  (local-set-key (kbd "r") 'prove-rename-buffer)
  (local-set-key (kbd "k") 'prove-get-testname)
...
(defun prove-rename-buffer ()
  "Simply, rename the current prove buffer to describe the
command (and uniquify)."
  (interactive)
  (rename-buffer
   (concat "*"  prove-mode-command " in " default-directory "*") t))
...
(easy-mmode-defmap prove-minibuffer-local-map
  '(("\C-i" . comint-dynamic-complete-filename))
  "Keymap for minibuffer prompting of prove startup command."
  :inherit minibuffer-local-map)

(defun prove-get-testname ()
  "Search for the last (closed) test filename and put it into the
kill ring."
  (interactive)
  (save-excursion
    (re-search-backward "^\\(t/[[:digit:]].+?\\)\\.\\{4\\}")
...
tap."
  (interactive
   (list (read-from-minibuffer "Run perl/prove (like this): "
                               (prove-build-command)
                               prove-minibuffer-local-map
                               nil
                               'prove-mode-history)))
  (compilation-start command-args 'prove-mode)
  (run-mode-hooks 'prove-mode-hook))
